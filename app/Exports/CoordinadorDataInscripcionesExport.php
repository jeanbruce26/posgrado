...
class CoordinadorDataInscripcionesExport implements FromCollection, WithHeadings, WithEvents
{
    ...
    */
    public function collection()
    {
        ...
        $inscripcion_pago = InscripcionPago::select('inscripcion.id_inscripcion','persona.nombre_completo','persona.num_doc','persona.celular1',InscripcionPago::raw('IF(persona.celular2 IS NULL, " ", persona.celular2)'),'persona.email',InscripcionPago::raw('IF(persona.email2 IS NULL, " ", persona.email2)'),'persona.especialidad')
    ...
    {
        return ["ID", "Apellidos y Nombres", "Documento", "Celular", "Celular Opcional", "Correo Electronico", "Correo Electronico Opcional", "Especialidad"];
    }
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function(AfterSheet $event) {
                ...
                $event->sheet->getStyle('A1:H1')->getFont()->setSize(11);
                ...
                $event->sheet->getStyle('A1:H1')->getFill()->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)->getStartColor()->setARGB('9dceff');
                $event->sheet->getStyle('A1:H1')->getFont()->getColor()->setARGB(\PhpOffice\PhpSpreadsheet\Style\Color::COLOR_BLACK);
                $event->sheet->getStyle('A1:H1')->getBorders()->getAllBorders()->setBorderStyle(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN);
                $event->sheet->setAutoFilter('A1:H1');
                $event->sheet->getStyle('A1:H1')->getAlignment()->setWrapText(true);
                ...
                $event->sheet->getStyle('A2:H'.$event->sheet->getHighestRow())->applyFromArray($styleArray);
                $event->sheet->getStyle('A2:H'.$event->sheet->getHighestRow())->getAlignment()->setWrapText(true);
                $event->sheet->getStyle('A2:H'.$event->sheet->getHighestRow())->getAlignment()->setVertical(\PhpOffice\PhpSpreadsheet\Style\Alignment::VERTICAL_CENTER);
