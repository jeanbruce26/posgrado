...
class UsersNoAdmitidosExport implements FromCollection, WithHeadings, WithEvents
{
    public function collection()
    {
        $no_admitidos = collect();
        $evaluaciones = Evaluacion::select('evaluacion.inscripcion_id','evaluacion.evaluacion_id', 'persona.nombre_completo','persona.num_doc', 'persona.direccion', 'sexo', 'fecha_naci', 'est_civil.est_civil', 'persona.email', 'persona.celular1', 'univer.universidad', 'persona.año_egreso', 'grado_academico.nom_grado', 'persona.especialidad', Admitidos::raw('IF(mencion.mencion IS NULL, CONCAT(CONCAT(programa.descripcion_programa," EN "), subprograma.subprograma), CONCAT(CONCAT(CONCAT(CONCAT(programa.descripcion_programa," EN "), subprograma.subprograma)," CON MENCION EN "), mencion.mencion)) as descripcion_programa'))
        ...
        foreach ($evaluaciones as $item) {
            $inscripcion_pago = InscripcionPago::join('pago','pago.pago_id','=','inscripcion_pago.pago_id')
                ->where('inscripcion_pago.inscripcion_id', $item->inscripcion_id)
                ->first();
            $no_admitidos->push([
                $item->inscripcion_id,
                ...
                $item->nom_grado,
                $item->especialidad,
                $inscripcion_pago->pago->nro_operacion,
                $inscripcion_pago->pago->monto,
                $inscripcion_pago->pago->fecha_pago,
    ...
    {
        return ["ID", "Apellidos y Nombres", "DNI", "Direccion", "Genero", "Fecha de Nacimiento", "Estado Civil", "Email", "Celular", "Universidad", "Año de Egreso", "Grado Academico", "Especialidad", "Nro Operacion", "Monto de Pago","Fecha de Pago", "Programa"];
    }
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function(AfterSheet $event) {
                ...
                $event->sheet->getStyle('P2:Q'.$event->sheet->getHighestRow())->getAlignment()->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_LEFT);
            },
        ];
    }
}
