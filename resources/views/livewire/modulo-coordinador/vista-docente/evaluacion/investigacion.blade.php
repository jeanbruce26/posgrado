<div>
    <div class="card">
        <div class="p-2 d-flex justify-content-between align-items-center">
            <a href="{{route('coordinador.docente.programas.inscripciones.index',$inscripcion->id_mencion)}}" type="button" class="btn btn-label w-md waves-effect waves-light fw-bold" style="background: rgb(151, 151, 151); color: white"><i class="ri-arrow-left-s-line label-icon align-middle fs-16 me-2"></i> Regresar</a>
            <h4 class="mt-2 text-center fw-bold">
                EVALUACIÓN DE PERFIL DE PROYECTO DE INVESTIGACIÓN
            </h4>
            <div class="w-md"></div>
        </div>
    </div>
            ...
            <span class="fs-5">
                ...
                <strong>
                    @if ($evaluacion_data->fecha_entrevista)
                        {{date('d/m/Y', strtotime($evaluacion_data->fecha_investigacion))}}
                    @else
    ...
    <div class="row">
        ...
        @php
            $expediente_tipo_evaluacion = App\Models\ExpedienteTipoEvaluacion::where('tipo_expediente_evaluacion', 2)->where('cod_exp', $item->cod_exp)->first();
        @endphp
            ...
            <div class="table-responsive table-card">
                <table class="table align-middle mb-0 table-hover table-nowrap table-bordered">
                    <thead style="background: rgb(199, 208, 219)">
                        <tr align="center">
                            <th class="col-md-6">PERFIL DE PROYECTO DE INVESTIGACION</th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td scope="row" align="justify" style="white-space: initial;">
                            ...
                            <td align="center">
                                <div>
                                    <button type="button" wire:click="cargarId({{$item->evaluacion_investigacion_item_id}})" class="btn btn-info" data-bs-toggle="modal" data-bs-target="#modalNota" @if ($boton != null) disabled @endif>Ingresar Puntaje</button>
                                </div>
                            </td>
                            <td align="center" class="fs-5">
                                @php
                                    $evaluacion_investigacion_notas = App\Models\EvaluacionInvestigacion::where('evaluacion_investigacion_item_id', $item->evaluacion_investigacion_item_id)->where('evaluacion_id',$evaluacion_id)->first();
                                @endphp
