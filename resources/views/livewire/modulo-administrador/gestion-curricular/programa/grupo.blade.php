                              ...
                              <tr>
                                  ...
                                  <td align="center">
                                      <div class="hstack gap-3 flex-wrap justify-content-center">
                                          <a href="#modalGrupo" wire:click="cargarGrupo({{ $item->id_grupo_programa }})" class="link-success fs-16" data-bs-toggle="modal" data-bs-target="#modalGrupo"><i class="ri-edit-2-line"></i></a>
                                      </div>
                                  </td>
                              </tr>
                              @endforeach
      ...
      <div class="modal-dialog">
          <div class="modal-content">
                  ...
                  <form novalidate>
                      <div class="row">
                          <div class="mb-3 col-md-12 col-sm-12">
                              ...
                              <input wire:model="grupo" type="text" class="form-control @error('grupo') is-invalid  @enderror" placeholder="Ingrese su grupo (ejemplo: A)">
                              @error('grupo') <span class="error text-danger" >{{ $message }}</span> @enderror
                          </div>
                          <div class="mb-3 col-md-12 col-sm-12">
                              <label class="form-label">Cantidad <span
                                  class="text-danger">*</span></label>
                              <input wire:model="cantidad" type="number" class="form-control @error('cantidad') is-invalid  @enderror" placeholder="Ingrese la cantidad">
                              @error('cantidad') <span class="error text-danger" >{{ $message }}</span> @enderror
                          </div>
                          <div class="mb-3 col-md-12 col-sm-12">
                              <label class="form-label">Admision <span
                                      class="text-danger">*</span></label>
                              <select wire:model="admision" class="form-select @error('admision') is-invalid  @enderror">
                                  <option value="">Seleccione su admision</option>
                                  @foreach ($admisiones as $item)
                                      <option value="{{ $item->cod_admi }}">{{ $item->admision }}</option>
                                  @endforeach
                              </select>
                              @error('admision') <span class="error text-danger" >{{ $message }}</span> @enderror
                          </div>
              ...
              <div class="modal-footer col-12 d-flex justify-content-between">
                  ...
                  <button type="button" wire:click="guardarGrupo()" class="btn btn-primary btn-label waves-effect right waves-light w-md"><i class="ri-check-double-fill label-icon align-middle fs-16 ms-2"></i> Guardar</button>
              </div>
          </div>
      </div>
  </div>
