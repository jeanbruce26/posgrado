	...
	<div class="col-sm-12">
		...
		<form action="{{ route('Universidad.update',$uni->cod_uni) }}" method="POST" class="row g-3">
               ...
               <div class="col-md-12">
                    ...
                    <input type="text" class="form-control" name="universidad"  value="{{ $uni->universidad }}" onkeypress="return soloLetras(event)">
                    @error('universidad')
			...
			<div class="col-md-6">
				...
				<input type="text" class="form-control" name="depart"  value="{{ $uni->depart }}" onkeypress="return soloLetras(event)">
                    @error('depart')
                         <div class="alert alert-danger mt-1 mb-1">{{ $message }}</div>
                    @enderror
               </div>
               <div class="col-md-6">
				...
				<input type="text" class="form-control" name="tipo_gesti"  value="{{ $uni->tipo_gesti }}" onkeypress="return soloLetras(event)">
                    @error('tipo_gesti')
                         <div class="alert alert-danger mt-1 mb-1">{{ $message }}</div>
                    @enderror
               </div>
